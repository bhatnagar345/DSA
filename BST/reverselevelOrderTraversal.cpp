...
void printReverseLevel(Node *p, int level)
{
    if(p== NULL)
    {
        return;
    ...
    if(level ==1){
        ...
        printReverseLevel(p->lchild,level-1);
        printReverseLevel(p->rchild,level-1);
        return;
    }
}
...
void ReverseLevel(Node *p)
{
    int h = height(p);
    for(int i=h;i>0;i--)
    {
        printReverseLevel(p,i);
    }
}
